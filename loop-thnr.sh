...
MAX_AGE_OF_RESPONSE_OBJECT_FILES_IN_SLASH_TMP_IN_MINUTES=1
...
cleanup_tmp_dir() {
  TARGET_DIR="/tmp"
  RO_DEST_DIR="/mnt/synology/datasets/hn-stories"
  ...
  while IFS= read -r file; do
    if ! mv "${file}" "${RO_DEST_DIR}"; then
      write-log-message loop error "${LOG_PREFIX_LOCAL} Failed to move ${file}"
    else
      ((num_ro_files += 1))
    fi
  ...
  write-log-message loop info "${LOG_PREFIX_LOCAL} Moved ${num_ro_files} response object files to ${RO_DEST_DIR}"
}
